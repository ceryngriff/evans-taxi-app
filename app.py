...
@app.route('/contracts/<int:contract_id>/edit', methods=['GET'])
@manager_required
def edit_contract(contract_id):
    c = Contract.query.get_or_404(contract_id)
    return render_template('edit_contract.html', contract=c)

@app.route('/update-contract/<int:contract_id>', methods=['POST'])
@manager_required
def update_contract(contract_id):
    c = Contract.query.get_or_404(contract_id)

    # Log what arrived (helpful when debugging)
    app.logger.info("Update contract %s form keys: %s", contract_id, list(request.form.keys()))
    ...
    c.school_postcode        = (request.form.get('school_postcode') or None)
    c.required_vehicle_size  = (request.form.get('required_vehicle_size') or None)

    start  = _parse_time(request.form.get('pickup_time'))   # HH:MM
    finish = _parse_time(request.form.get('school_time'))   # HH:MM
    if start  is not None: c.route_start_time  = start
    if finish is not None: c.route_finish_time = finish
    ...
    def _get(seq, i, default=""):
        return (seq[i] if i < len(seq) else default) or default

    if ids:
        # Update by primary key; delete any removed
        ...
        for i in range(len(names)):
            ...
            if ch:
                ...
                db.session.add(Child(name=name, address=addr,
                                     child_postcode=pc, contract_id=c.id))
        # Remove children that were deleted in the form
        for cid, ch in current_by_id.items():
            if cid not in seen:
                db.session.delete(ch)
    else:
        # No IDs provided — replace the current set with the submitted rows
    ...
    try:
        ...
        db.session.rollback()
        app.logger.exception("Contract update failed")
